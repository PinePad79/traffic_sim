...
FEATURE_MAP = {
    ...
    "DNS Filtering Test": {
        "feature": "DNS Filter / FortiGuard DNS rating / Logging",
        "notes": "Envía queries DNS A por UDP/53 a un DNS server (ideal: FortiGate DNS proxy). Si la respuesta incluye 208.91.112.55 se considera bloqueado por FortiGuard DNS Filtering."
    },
    "IPS Test": {
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # DNS server for DNS Filtering Test (UDP/53)
        ttk.Label(top, text="DNS Server (UDP/53):").grid(row=4, column=0, sticky="w", pady=(6, 2))
        self.dns_var = tk.StringVar(value=self._get_default_dns_server())
        ttk.Entry(top, textvariable=self.dns_var, width=22).grid(row=4, column=1, sticky="w", padx=6, pady=(6, 2))
        ttk.Label(top, text="(Tip: use FortiGate DNS Proxy IP)").grid(row=4, column=2, columnspan=3, sticky="w", pady=(6, 2))
        ...
        self._add_action_button(left, "DNS Filtering Test", self._action_dns_filter_test)
    ...
    def _get_default_dns_server(self) -> str:
        # Try to read first nameserver from /etc/resolv.conf (fallback to empty)
        try:
    ...
    def _action_dns_filter_test(self):
        sc = self._current_scenario()
        dns_server = self.dns_var.get().strip() if hasattr(self, "dns_var") else ""
        if not dns_server:
            messagebox.showwarning("DNS Server required", "Please enter a DNS server IP (UDP/53). Tip: FortiGate DNS Proxy IP.")
            return

        self._exec_by_mode(
            "dnstest",
            lambda: ["./dns_test.sh", dns_server],
            "DNS Filtering Test",
            sc
        )
