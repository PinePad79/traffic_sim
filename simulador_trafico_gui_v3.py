...
FEATURE_MAP = {
    ...
    "IPS Test": {
        "feature": "IPS (custom signature) / Security Profiles / Logging",
        "notes": "Request con URI/UA específicos para disparar una firma IPS de laboratorio."
    },
    "IOC Beacon": {
...
class App(tk.Tk):
    ...
    def _action_ips_test(self):
        ...
        if not self._target_allowed(sc.target_ip):
            return
        self._exec_by_mode("ips", lambda: ["./ips_test.sh", sc.target_ip, sc.path_ips_test, sc.user_agent, str(sc.curl_timeout_sec)], "IPS Test", sc)
